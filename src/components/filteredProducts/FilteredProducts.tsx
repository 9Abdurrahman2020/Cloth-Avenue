...
const FilteredProducts = () => {
    ...
    const [ searchData, setSearchData ] = useState<ISearch>({...filter, price: "50"});
    ...
    useEffect( ()=>{
        ...
        .then( (data:IProduct[]) => {
            setAllProduct(data)
            setProducts(data.filter(pd => pd.for === filter.department))
        })
    },[filter.department]);
    ...
    return (
        <div>
            <div>
                <h1 className='category-banner shadow'>{filter.department}</h1>
            </div>
            <Container className="category-based-product-container">
                <Row className="g-4">
                    <div className="col-md-3 ">
                        ...
                        <div className={`${ !filterBtn && 'mobile-filter-box'}`}>
                            ...
                            <Form.Group className="mb-3">
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Select onChange={onInputChangeHandler} defaultValue={`${filter?.type ? filter?.type : ''}`} name="type" required>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Select onChange={onInputChangeHandler} defaultValue={`${filter?.brand ? filter?.brand : ''}`} name="brand" disabled={ !department && true} required>
                    ...
                    <div className="col-md-9">
                        ...
                        <Row>
                            {
                               products.length > 0 ? products?.map( pd=> <ProductCard key={pd._id} data={pd} category={filter.department}/>) : <h3 className="text-danger text-center my-5"> No Product Found !</h3>
                            }
                        </Row>
                    </div>
                </Row>
